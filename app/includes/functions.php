<?php
	...
	function obtener_clientes($client)
	{
		global $conexion;
		$consulta = "SELECT * 
					FROM personal_info ";
		if($client)
		{
			$consulta .= "WHERE client1=". $client1 . " AND client2=".$client2;
		}
		
	  	$clientes = mysql_query($consulta,$conexion);
		verificar_consulta($clientes);
		return $clientes;
	}
	
	function obtener_info_financiera($financial)
	{
		global $conexion;
		$consulta = "SELECT * 
					FROM financial_info ";
		if($financial)
		{
	  	...
	  	$financial_info = mysql_query($consulta,$conexion);
		verificar_consulta($clientes);
